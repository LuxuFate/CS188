...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = range(0, (X_RANGE-1)+(Y_RANGE-1)+MAX_NOISE+1)
    variableDomainsDict[OBS1] = range(0, (X_RANGE-1)+(Y_RANGE-1)+MAX_NOISE+1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        probList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for e in eliminationOrder:
            probList, joined = joinFactorsByVariable(probList, e)
            if len(joined.unconditionedVariables()) > 1:
                probList.append(eliminate(joined, e))
        return normalize(joinFactors(probList))
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for item in self.items():
            array.append(array[len(array)-1] + item[1])
            keys.append(item[0])
        for val in array:
            if sam < val:
                return keys[array.index(val)]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            self.beliefs[ghostPos] = self.beliefs[ghostPos] * self.getObservationProb(observation, pacPos, ghostPos, jailPos)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for new in newPosDist.items():
                newB[new[0]] += self.beliefs[oldPos] * newPosDist[new[0]]
        self.beliefs = newB
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        perPos = self.numParticles//len(self.legalPositions)
        leftovers = self.numParticles%len(self.legalPositions)
        self.particles = (perPos * self.legalPositions)
        self.particles.extend(self.legalPositions[0:leftovers-1])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.particles:
            belief[p] += self.getObservationProb(observation, pacPos, p, jailPos)
        belief.normalize()
        if belief.total() == 0:
            self.initializeUniformly(gameState)
            return
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            newPar.append(newPosDist.sample())
