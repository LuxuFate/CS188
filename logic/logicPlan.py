...
def sentence3() -> Expr:
    ...
    return conjoin([one, two, PacmanBorn_0])
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combo = itertools.combinations(literals, 2)
    pairs = [~exp[0] | ~exp[1] for exp in list(combo)]
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in non_outer_wall_coords:
        allSquares.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(allSquares))
    dirs = []
    for dir in DIRECTIONS:
        dirs.append(PropSymbolExpr(dir, time=t))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, 
    ...
    model1 = PropSymbolExpr(pacman_str, x1, y1, time=1)
    model2 = ~PropSymbolExpr(pacman_str, x1, y1, time=1)
    return (findModel(conjoin(KB + [model1])), findModel(conjoin(KB + [model2])))
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            squares.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(squares))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        leftover = []
        for x, y in food:
            leftover.append(PropSymbolExpr(food_str, x, y, time=t))
        eaten = []
        for f in leftover:
            eaten.append(~f)
        model = findModel(conjoin(KB + [conjoin(eaten)]))
        if model:
            return extractActionSequence(model, actions)
        acts = []
        for act in actions:
        ...
        for x, y in food:
            ...
            KB.append(~food_t1 % disjoin([~food_t, conjoin([pacman_t, food_t])]))
    "*** END YOUR CODE HERE ***"
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            if entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y)): 
                ...
                known_map[x][y] = 1
            if not (entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)) or entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y))):
                known_map[x][y] = -1
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield known_map
